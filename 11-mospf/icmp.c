...
void icmp_send_packet(const char *in_pkt, int len, u8 type, u8 code)
{
	...
	log(DEBUG, "sending icmp packet, type = %d, code = %d, src = %x, dest = %x", type, code, ntohl(in_ip->daddr), ntohl(in_ip->saddr));
	...
	if (type == ICMP_ECHOREPLY) {
		...
		if (!entry) {
			...
			log(DEBUG, "about to free packet at icmp_send_packet");
			free(out_pkt);
			return ;
		}
