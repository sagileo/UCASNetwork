...
void tcp_process(struct tcp_sock *tsk, struct tcp_cb *cb, char *packet)
{
	...
	{	// it's server parent socket receiving SYN packet when passively set up connection
		...
		if(tsk->rcv_nxt > cb->seq) {
			log(WARNING, "received OUT-OF-DATE packet, drop it\n");
			return;
		} else if(tsk->rcv_nxt < cb->seq) {
			// log(DEBUG, "received OUT-OF-ORDER packet, pend it\n");
		...
		// 				exit(1);
		// 			}
		// 			int seq_end = tcp->seq + data_len + ((tcp->flags & (TCP_SYN|TCP_FIN)));
		// 			tsk->rcv_nxt = seq_end;
		// 			tsk->rcv_wnd = ring_buffer_free(tsk->rcv_buf);
