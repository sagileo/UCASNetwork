...
{
	int data_off = ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE;
	int left = len;
	while(1)
	{
		int data_len = min(left, ETH_FRAME_LEN - ETHER_HDR_SIZE - IP_BASE_HDR_SIZE - TCP_BASE_HDR_SIZE);
		char *packet = (char*)malloc(data_off + data_len);
		memcpy(packet + data_off, buf, data_len);
		tcp_send_packet(tsk, packet, data_off + data_len);
		left -= data_len;
		buf += data_len;
		if(left == 0)
			break;
...
void tcp_send_packet(struct tcp_sock *tsk, char *packet, int len) 
{
	...
	char *packet_cp = (char*)malloc(len);
	memcpy(packet_cp, packet, len);
	struct cached_pkt *pkt = (struct cached_pkt*)malloc(sizeof(struct cached_pkt));
	pkt->len = len;
	pkt->packet = packet_cp;
	list_add_tail(&(pkt->list), &(tsk->send_buf));
	// printf("DEBUG: tcp data len: %d, seq: %d, ack: %d\n", tcp_data_len, seq, ack);
...
{
	// printf("DEBUG: sended tcp_control_packet with flags: 0x%x\n", flags);
