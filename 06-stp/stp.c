...
bool stp_is_root_switch(stp_t *stp)
{
...
bool prior_config_port(struct stp_config *config, stp_port_t *p)
{
	if(ntohll(config->root_id) > p->designated_root)
		return false;
	else if(ntohll(config->root_id) == p->designated_root && ntohl(config->root_path_cost) > p->designated_cost)
		return false;
	else if(ntohll(config->root_id) == p->designated_root && ntohl(config->root_path_cost) == p->designated_cost && ntohll(config->switch_id) > p->designated_switch)
		return false;
	else if(ntohll(config->root_id) == p->designated_root && ntohl(config->root_path_cost) == p->designated_cost && ntohll(config->switch_id) == p->designated_switch && ntohs(config->port_id) > p->designated_port)
		return false;
	
	return true;
}

bool prior_port_port(stp_port_t *p1, stp_port_t *p2)
{
		...
		struct stp_config *config)
{
	...
	if(prior_config_port(config, p) == false)
	{
		stp_port_send_config(p);
		return;
	}
	...
	// find the root port of this switch
	stp_port_t *pi = NULL, *pr = NULL, pl;
	pl.designated_cost = INT32_MAX;
	pl.designated_port = INT32_MAX;
	pl.designated_root = UINT64_MAX;
	pl.designated_switch = UINT64_MAX;
	pr = &pl;
	for (int i = 0; i < stp->nports; i++) {
		pi = &stp->ports[i];
		if (stp_port_is_designated(pi) == false && prior_port_port(pi, pr)) {
			pr = pi;
		}
	}

	// renew this switch
	if(pr == &pl)	// root port does not exist, then this switch is root switch
	{
		stp->root_port = NULL;
		stp->designated_root = stp->switch_id;
		stp->root_path_cost = 0;
	}
	else		// root port is pr
	{
		stp->root_port = pr;
		stp->designated_root = pr->designated_root;
		stp->root_path_cost = pr->designated_cost + pr->path_cost;
	}

	if(stp->root_port != NULL)
		stp_stop_timer(&(stp->hello_timer));

	// renew ports of this switch
	for (int i = 0; i < stp->nports; i++) {
		pi = &stp->ports[i];
		
		if(stp_port_is_designated(pi) == false) {
			bool prior = (pi->designated_root > stp->designated_root) 
					|| (pi->designated_root == stp->designated_root && pi->designated_cost > stp->root_path_cost)
					|| (pi->designated_root == stp->designated_root && pi->designated_cost == stp->root_path_cost && pi->designated_switch > stp->switch_id)
					|| (pi->designated_root == stp->designated_root && pi->designated_cost == stp->root_path_cost && pi->designated_switch == stp->switch_id && pi->designated_port > pi->port_id);
			if(prior == true)
			{
				pi->designated_root = stp->designated_root;
				pi->designated_cost = stp->root_path_cost;
				pi->designated_switch = stp->switch_id;
				pi->designated_port = pi->port_id;
			}
		}
		else if(stp_port_is_designated(pi) == true)
		{
			pi->designated_root = stp->designated_root;
			pi->designated_cost = stp->root_path_cost;
		}
